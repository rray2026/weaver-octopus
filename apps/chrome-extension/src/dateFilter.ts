...
export const DEFAULT_FILTER: DateFilter = { type: 'today' };
...
export async function saveFilter(filter: DateFilter): Promise<void> {
  await chrome.storage.local.set({ [STORAGE_KEY]: filter });
}

export interface DateRange {
  /** Inclusive ms timestamp at start of range (local). */
  start: number;
  /** Exclusive ms timestamp at end of range (local). */
  end: number;
  /** Human-readable label, e.g. "2026-04-29" or "2026-04-22 → 2026-04-29". */
  label: string;
}
...
export function computeRange(filter: DateFilter, now: Date): DateRange {
  ...
  switch (filter.type) {
    ...
    case 'thisWeek': {
      // ISO week: Monday is day 1; getDay() returns 0 for Sunday.
